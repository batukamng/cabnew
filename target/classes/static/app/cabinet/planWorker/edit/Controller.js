angular.module("altairApp")
    .controller("workerPlanEditCtrl", [
        ...
        "objectives",
        "__env",
        function ($rootScope, $state,$stateParams, $scope, $timeout, mainService, commonDataSource, sweet, item,objectives, __env) {
            ...
            $scope.objectives2 = objectives;
            $scope.selectedTab = 'tab1';
            $scope.editable = false;
            if(item.userStatusNm==='Хадгалсан' || item.userStatusNm==='Цуцалсан'){
                $scope.editable = true;
            }
            $scope.bags = [];
            $scope.workers = [];
            ...
            $scope.loadEvent = function (){
                mainService.withResponse("get", "/api/admin/v1/multiple/cab-plan-user-objective/"+$scope.app.id+"/"+$scope.user.id).then(function (response) {
                    if(response.status===200){
