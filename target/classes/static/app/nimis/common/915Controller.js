            ...
            function ($rootScope, $state, $scope, $timeout, $translate, commonDataSource,mainService,__env) {
                ...
                $scope.mainGrid = {
                    ...
                    columns: [
                        ...
                        { template:"#if(iconImage!=null){# <div class='avatar-photo' style='background-image: url(#=iconImage.uri#);'></div> #}#", filterable: { cell: { operator: "contains", showOperators: false } }, headerAttributes: { "class": "columnHeader" }, title: 'Икон' },
                        ...
                        { field: "useYn", headerAttributes: { "class": "columnHeader" }, width: 130, template: "#if(useYn===1){# <span class='columnCenter'>Y</span> #}else{#<span class='columnCenter uk-text-danger'>N</span> #}#", title: 'Ашиглах эсэх' }
                    ],
                ...
                $scope.dataSubmit=function (){
                    if ($scope.validator.validate()) {
                        ...
                        mainService.withdata('post', __env.apiUrl() + '/api/nms/sector/submit', $scope.dataItem)
                            .then(function (data) {
                                    modalForm.hide();
                                    if($scope.dataItem.parentId==null){
                ...
                $scope.add = function (item){
                    if(item!=null){
                        $scope.dataItem={parentId:item.id,useYn:1};
                    }
                    else {
                ...
                if (localStorage.getItem('buttonData').includes("update") || localStorage.getItem('buttonData').includes("edit")) {
                    $scope.mainGrid.columns.push({
                        command: [
                            {
                                template:
                                    '<div class="command-container"><a class="grid-btn" ng-click="update(dataItem)"><div class="nimis-icon edit"></div></a><a class="grid-btn k-grid-remove-command"><div class="nimis-icon delete"></div></a><a class="grid-btn k-grid-update"><div class="nimis-icon update"></div></a> <a class="grid-btn k-grid-cancel"><div class="nimis-icon cancel"></div></a></div>',
                            },
                ...
                $scope.detailGridOptions = function (dataItem) {
                    return {
                        ...
                        pageable: false,
                        toolbar: [
                            { template: "<button class='md-btn custom-btn' ng-click='add(dataItem)'><i class='material-icons text-white mr-1'>add</i>Нэмэх</button>" },
              ...
              /*  $scope.thirdGridOptions = function (dataItem) {
                    console.log("thirdGridOptions dataItem.id=>" + dataItem.id);
                    return {
                        dataSource: {
                            transport: {
                                read: {
                        ...
                        columns: [
                            ...
                            { field: "useYn", headerAttributes: { "class": "columnHeader" }, width: 130, template: "#if(useYn===1){# <span class='columnCenter'>Y</span> #}else{#<span class='columnCenter uk-text-danger'>N</span> #}#", title: '{{"Cmn06" | translate}}' },
                            {
                                command: [
                                    {
                                        template:
