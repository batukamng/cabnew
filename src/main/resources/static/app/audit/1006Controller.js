    ...
    .controller("1006AdtCtrl", [
        ...
        function ($rootScope, $state, $scope, $timeout, mainService, commonDataSource, sweet, Upload, __env) {
            ...
            $scope.appDataSource = new kendo.data.DataSource({
                transport: {
                    read: {
                        url: __env.apiUrl() + "/api/adt/try/out/list",
                    ...
                    update: {
                        url: __env.apiUrl() + "/api/adt/try/out/submit",
                        contentType: "application/json; charset=UTF-8",
                        type: "POST",
                        beforeSend: function (req) {
                            req.setRequestHeader('Authorization', 'Bearer ' + JSON.parse(localStorage.getItem('currentUser')).token);
                    ...
                    destroy: {
                        url: __env.apiUrl() + "/api/adt/try/out",
                        contentType: "application/json; charset=UTF-8",
                        type: "DELETE",
                        beforeSend: function (req) {
                            req.setRequestHeader('Authorization', 'Bearer ' + JSON.parse(localStorage.getItem('currentUser')).token);
            ...
            $scope.formSubmit = function () {
                ...
                if (validator.validate()) {
                    ...
                    mainService.withdata(method, "/api/adt/try/out/submit", $scope.dataItem).then(function (data) {
