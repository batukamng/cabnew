        ...
        [
            ...
            function ($rootScope, $state, $scope, $filter, $timeout, mainService, Upload, commonDataSource, sweet, __env) {
                ...
                $scope.activeTab = function (item) {
                    ...
                    if(item.code==='СБД'){
                        $scope.dataSource32.filter({
                            logic: "and",
                            filters: [{field: "useYn", operator: "eq", value: 1},{field: "stepId", operator: "eq", value: 0}, {
                                field: "planId",
                                operator: "eq",
                                value: $scope.appId
                            }],
                ...
                $scope.submitUploadDifference = function () {
                    $scope.sendBtn = false;
                    $scope.uploadReport32($scope.afl, 1);
                };
                $scope.submitUploadReport32 = function () {
                    $scope.sendBtn = false;
                    $scope.uploadReport32($scope.uploadfile, 2);
                };
                $scope.submitUploadJournal = function () {
                    $scope.sendBtn = false;
                    $scope.uploadReport32($scope.tsalin, 3);
                };
                $scope.submitUploadHuulga = function () {
                    $scope.sendBtn = false;
                    if ($scope.formHuulga.huulga.$valid && $scope.huulga) {
                        $scope.uploadReport32($scope.huulga, 4);
                    }
                };
                $scope.uploadReport32 = function (file, y) {
                    UIkit.modal('#modal_loader_32', {
                        modal: false,
                        keyboard: false,
                        bgclose: false,
                        center: true
                    ...
                    if (y === 1) {
                        ...
                        xurl = '/api/adt/main/convert/' + $scope.appId + '/0';
                    } else if (y === 3) {
                    ...
                    Upload.upload({
                        ...
                        if (y === 1) {
                            ...
                            UIkit.modal('#modal_loader_32').hide();
                        } else if (y === 2) {
                            if(!response.data.valid){
                                $rootScope.alert(false,"Тайлан тохирохгүй байна-1.");
                            }
                            UIkit.modal('#modal_loader_32').hide();
                        } else if (y === 3) {
                            if(!response.data.valid){
                                $rootScope.alert(false,"Тайлан тохирохгүй байна2-2.");
                            }
                            UIkit.modal('#modal_loader_32').hide();
                        }
