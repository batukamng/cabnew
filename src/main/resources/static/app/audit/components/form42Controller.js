    ...
    .controller(
        ...
        [
            ...
            function ($rootScope, $state, $scope, $filter, $timeout, mainService, fileUpload, commonDataSource, sweet, __env) {
                ...
                $scope.$on("formChanged42", function (event, args, data) {
                ...
                $scope.initForm = function (appId, formId) {
                    $scope.appId = appId;
                    $scope.formId = formId;
                    mainService.withdomain('get', __env.apiUrl() + '/api/adt/main/materiality/1/' + $scope.appId)
                        .then(function (data) {
                                $scope.materialityList = data;
                            }
                        );
                    mainService.withdomain('get', __env.apiUrl() + '/api/adt/main/item/materiality/1/' + $scope.appId)
                        .then(function (data) {
                                if (data.length > 0) {
                                    $timeout(function (){
                ...
                $scope.form={};
                $scope.formSubmit42= function (){
                    var validator = $("#materiality42").kendoValidator().data("kendoValidator");
                    if (validator.validate()) {
                        sweet.show(
                            ...
                            function (inputvalue) {
                                if (inputvalue) {
                                    UIkit.modal("#modal_loader_42", {
                                        modal: false,
                                        keyboard: false,
                                        bgclose: false,
                                        center: true,
                                    }).show();
                                    $scope.materiality.noteId=1;
                                    ...
                                    mainService.withdata('post', __env.apiUrl() + '/api/adt/main/materiality/submit',$scope.materiality)
                                        .then(function (data) {
                                                ...
                                                $timeout(function (){
                                                    UIkit.modal("#modal_loader_42").hide();
                                                });
                                            }
                                        );
                                }
