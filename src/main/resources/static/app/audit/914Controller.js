    ...
    .controller("914NmsCtrl", [
        ...
        function ($rootScope, $state, $scope, $timeout, mainService, commonDataSource, Upload, $http, __env) {
            ...
            $scope.dataSource = new kendo.data.DataSource({
                transport: {
                    read: {
                        url: "/api/adt/governor/list",
                        contentType: "application/json; charset=UTF-8",
                        type: "POST",
                        data: {sort: [{field: "shortName", dir: "asc"}]},
                    ...
                    destroy: {
                        url: __env.apiUrl() + "/api/adt/governor",
                        contentType: "application/json; charset=UTF-8",
                        type: "DELETE",
                        beforeSend: function (req) {
                            req.setRequestHeader("Authorization", "Bearer " + JSON.parse(sessionStorage.getItem("currentUser")).token);
            ...
            $scope.mainGrid = {
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        dataTextField: "comCdNm",
                        dataValueField: "id",
                        dataSource: $scope.tezTypeDataSource,
                        template: "<span>#if(commonCd!=null){# #=commonCd.comCdNm# #}#</span>",
                        width: 150
                    },
                ],
            ...
            $scope.dataSubmit = function () {
                if ($scope.validator.validate()) {
                    $scope.dataItem.sectorsArr = [];
                    for (let menuKey in $scope.dataItem.sectorObj) {
                        if ($scope.dataItem.sectorObj[menuKey]) $scope.dataItem.sectorsArr.push(menuKey);
                    }

                    mainService.withdata("post", __env.apiUrl() + "/api/nms/general/governor/submit", $scope.dataItem).then(function (data) {
                        modalForm.hide();
            ...
            $scope.formSubmit = function (type) {
                ...
                if (validator.validate()) {
                    ...
                    mainService.withdata(method, "/api/adt/governor", $scope.dataItem).then(function (data) {
