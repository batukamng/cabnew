        ...
        [
            ...
            function ($rootScope, $state, $scope, $timeout, mainService, commonDataSource, fileUpload, __env) {
                    ...
                    $scope.spoDataSource = new kendo.data.DataSource({
                        transport: {
                            read: {
                                ...
                                contentType: "application/json; charset=UTF-8",
                                data: { "custom": "where typeId=2 and useYn=true ", filter: { logic: "and", filters: arr } },
                                type: "POST",
                                beforeSend: function (req) {
                                    req.setRequestHeader('Authorization', 'Bearer ' + JSON.parse(sessionStorage.getItem('currentUser')).token);
                                }
                ...
                $scope.checkEditor = function (container, options) {
                    ...
                    $scope.checkDataSource = new kendo.data.DataSource({
                        transport: {
                            read: {
                                url: __env.apiUrl() + "/api/org/list",
                                contentType: "application/json; charset=UTF-8",
                                data: { "custom": "where typeId=1 and useYn=true ", filter: { logic: "and", filters: arr } },
                                type: "POST",
                                beforeSend: function (req) {
                                    req.setRequestHeader('Authorization', 'Bearer ' + JSON.parse(sessionStorage.getItem('currentUser')).token);
                                }
                    ...
                    var editor = $('<select kendo-drop-down-list  k-filter="\'startswith\'" k-data-text-field="\'name\'" k-data-value-field="\'id\'" k-data-source="checkDataSource" data-bind="value:' + options.field + '"></select>')
                        .appendTo(container);
                };
                $scope.governorEditor = function (container, options) {
                    ...
                    $scope.governorDataSource = new kendo.data.DataSource({
                        transport: {
                            read: {
                                ...
                                contentType: "application/json; charset=UTF-8",
                                data: { "custom": "where useYn=true  and ordNo<33", filter: { logic: "and", filters: arr } },
                                type: "POST",
                                beforeSend: function (req) {
                                    req.setRequestHeader('Authorization', 'Bearer ' + JSON.parse(sessionStorage.getItem('currentUser')).token);
                                }
                    ...
                    var editor = $('<input kendo-drop-down-list k-filter="\'startswith\'" k-data-text-field="\'name\'" k-data-value-field="\'id\'" k-data-source="governorDataSource" data-bind="value:' + options.field + '"/>')
                        .appendTo(container);
                };
                        ...
                        dataSource: {
                            transport: {
                                read: {
                                    ...
                                    contentType: "application/json; charset=UTF-8",
                                    type: "POST",
                                    data: { "custom": "where auth in ('Tra0201','Nimis')", "sort": [{ field: 'id', dir: 'desc' }] },
                                    beforeSend: function (req) {
                                        req.setRequestHeader('Authorization', 'Bearer ' + JSON.parse(sessionStorage.getItem('currentUser')).token);
                                    }
                                },
                ...
                $scope.govFilterDataSource = commonDataSource.urlPageDataSource("/api/gen/gov/list", JSON.stringify({ "custom": "where useYn=true  and ordNo<33" }), 300);
                $scope.spoFilterDataSource = commonDataSource.urlPageDataSource("/api/org/list", JSON.stringify({ "custom": "where typeId=2 and useYn=true" }), 30);
                $scope.amgFilterDataSource = commonDataSource.urlPageDataSource("/api/as/cd/list", JSON.stringify({ "custom": "where length(asCd)=2 and useYn=true", sort: [{ field: "cdNm", dir: "asc" }] }), 30);
                $scope.checkFilterDataSource = commonDataSource.urlPageDataSource("/api/org/list", JSON.stringify({ "custom": "where typeId=1 and useYn=true" }), 30);
                ...
                $scope.mainGrid = {
                    dataSource: {
                        transport: {
                            read: {
                                ...
                                contentType: "application/json; charset=UTF-8",
                                type: "POST",
                                data: { "custom": "where userType=2", "sort": [{ field: 'id', dir: 'desc' }] },
                                beforeSend: function (req) {
                                    req.setRequestHeader('Authorization', 'Bearer ' + JSON.parse(sessionStorage.getItem('currentUser')).token);
                                }
                            },
                    ...
                    columns: [
                        ...
                        {
                            field: "tezId", sticky: true,
                            dataTextField: "name", dataValueField: "id", dataSource: $scope.govFilterDataSource,
                            headerAttributes: { "class": "columnHeader" }, width: 200, template: "#if(governor!=null){# <span>#=governor.name#</span> #}#", editor: $scope.governorEditor, title: 'Төсвийн ерөнхийлөн захирагч'
                        },
                        {
                            field: "spoId", stickable: true,
                            dataTextField: "name", dataValueField: "id", dataSource: $scope.spoFilterDataSource,
                            filterable: { cell: { operator: "contains", showOperators: false } },
                            headerAttributes: { "class": "columnHeader" }, width: 200, template: "#if(sponsor!=null){# <span>#=sponsor.name#</span> #}#", editor: $scope.sponsorEditor, title: 'Эрх шилжүүлсэн байгууллага'
                        },
                        { field: "amgId", dataTextField: "cdNm", dataValueField: "id", dataSource: $scope.amgFilterDataSource, stickable: true, headerAttributes: { "class": "columnHeader" }, width: 200, template: "#if(aimag!=null){# <span>#=aimag.cdNm#</span> #}#", editor: $scope.aimagEditor, title: 'Аймаг/Нийслэл' },
                        { field: "checkId", dataTextField: "name", dataValueField: "id", dataSource: $scope.checkFilterDataSource, stickable: true, headerAttributes: { "class": "columnHeader" }, width: 200, template: "#if(check!=null){# <span>#=check.name#</span> #}#", editor: $scope.checkEditor, title: 'Хянагч байгууллага' },
