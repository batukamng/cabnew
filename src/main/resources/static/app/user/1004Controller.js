        ...
        [
            ...
            function ($rootScope, $state, $scope, $timeout, mainService, fileUpload, __env) {
                ...
                $scope.ministerEditor = function (container, options) {
                    var roles = $scope.selectedIds = [];
                    angular.forEach(options.model.ministers, function (value, key) {
                        roles.push(value.id);
                    });
                    options.field = "ministers";
                    ...
                    var editor = $('<select  kendo-multi-select  k-options="minOptions" required data-bind="value:' + options.field + '"></select>')
                        .appendTo(container);
                };
                ...
                $scope.mainGrid = {
                    ...
                    columns: [
                        ...
                        { field: "ministers", stickable: true, width: 200, headerAttributes: { "class": "columnHeader" }, editor: $scope.ministerEditor, filterable: false, template: "#if(lnkUserMinister!=null){# #for (var i=0,len=lnkUserMinister.length; i<len; i++){#<span>${ lnkUserMinister[i].name } #if(i!=lnkUserMinister.length-1){#<span>,</span>#}# </span> # } # #}# ", title: 'Яам' },
