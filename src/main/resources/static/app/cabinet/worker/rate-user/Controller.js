    ...
    .controller("rateUserCtrl", [
        ...
        function ($rootScope, $state, $scope, $timeout, mainService, commonDataSource, sweet, Upload, __env) {
            ...
            $scope.addMeeting = function (item) {
                $scope.viewItem=item;
                $scope.dataItem = {useYn: 1,criteria:item.criteria,detId:item.detId, criteriaId:item.id,planId:item.planId,detNm:item.title,userId:$scope.user.id, progress:0,planYr:$scope.planYr,eval:item.eval,apr:item.apr};
            ...
            $scope.addComplain = function (item) {
                $scope.viewItem=item;
                $scope.dataItem = {useYn: 1,criteria:item.criteria,detId:item.detId, criteriaId:item.id,planId:item.planId,detNm:item.title,userId:$scope.user.id, progress:0,planYr:$scope.planYr,eval:item.eval,apr:item.apr,status:"sent"};
                $timeout(() => $rootScope.clearForm("userForm"));
