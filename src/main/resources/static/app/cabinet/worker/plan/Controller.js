    ...
    .controller("planWorkerCtrl", [
        ...
        function ($rootScope, $stateParams,$state, $scope, $timeout, mainService, commonDataSource, sweet, Upload, __env) {
            ...
            $scope.removePurpose = function (item) {
                $scope.removeItem = {useYn: 1, detId:item.id,planId:item.planId,userId:$scope.app.userId};
            ...
            $scope.formRemoveSubmit = function () {
                ...
                if (validator.validate()) {
                    var method = "post";
                    $scope.removeItem.userType=$scope.user.level.name;
                    ...
                    mainService.withdata("post", "/api/cab/plan/detail/request/submit", $scope.removeItem).then(function (data) {
                        modalRemove.hide();
                        $scope.loadDetail($scope.removeItem.planId,$scope.removeItem.userId);
                    });
                }
            };
            ...
            {
                $scope.formTp=tp;
                $scope.detailItem = {useYn: 1,planId:item.planId, objType:objType,eventType:'01',userType:'02',userId:$scope.app.userId,criterias:[{}]};
            ...
            $scope.formDetailSubmit = function () {
                ...
                if($scope.detailItem.criterias.length>0){
                    for(var i=0; i<$scope.detailItem.criterias.length;i++){
                        ...
                        const d = new Date($scope.detailItem.criterias[i].endDt);
                        let month = d.getMonth();
                        if((month<6 && $scope.detailItem.criterias[i].firstHalf>0) || (month>5 && $scope.detailItem.criterias[i].secondHalf>0)){
                            crits.push($scope.detailItem.criterias[i]);
                        }
                        else{
                            $rootScope.alert(false,"Хүрэх түвшин шалгана уу");
                        }
                    }
                    if(crits.length>0){
                ...
                if (validator.validate() && crits.length>0) {
                    var method = "post";
                    ...
                    mainService.withdata("post", "/api/cab/plan/detail/user/submit", $scope.detailItem).then(function (data) {
                        modalDetail.hide();
                        $scope.loadDetail($scope.detailItem.planId,$scope.detailItem.userId);
                    });
                }
            };
            ...
            $scope.deletePurpose=function (item){
                ...
                sweet.show(
                    ...
                    function (inputvalue) {
                        if (inputvalue) {
                            mainService.withdomain("delete", "/api/cab/plan/objective/"+item.id).then(function (data) {
                                $scope.loadEvent($scope.purposeItem.planId,$scope.purposeItem.userId);
                            });
                        }
                    }
                );
