        ...
        function ($rootScope, $scope, $state, $timeout, sweet, __env, commonDataSource, mainService) {
            ...
            $scope.dataSource = new kendo.data.DataSource({
                transport: {
                    read: {
                        url: function (e) {
                            return __env.apiUrl() + "/api/notification/main/list";
                        },
            ...
            if (sessionStorage.getItem("buttonData").includes("update") || sessionStorage.getItem("buttonData").includes("edit")) {
                $scope.mainGrid.columns.push({
                    command: [
                        {
                            template:
                                '<div class="flex gap-3"><button style="margin-left: 10px" class="grid-btn k-grid-edit" ng-click=\'update(dataItem)\'><div class="nimis-icon edit"></div></button><button class="grid-btn k-grid-delete" ng-click=\'delete(dataItem)\'><div class="nimis-icon delete"></div></button></div>',
                        },
                    ],
                    title: "&nbsp;",
                    width: 90,
            ...
            $scope.delete = function (item) {
                sweet.show(
                    ...
                    function (inputvalue) {
                        if (inputvalue) {
                            mainService.withdomain("delete", __env.apiUrl() + "/api/notification/delete/"+ item.id).then(function (data) {
                                $rootScope.alert(true, "Устлаа.");
                                $(".k-grid").data("kendoGrid").dataSource.read();
                            });
                        }
                    }
