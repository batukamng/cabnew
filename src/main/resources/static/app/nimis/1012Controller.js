angular.module("altairApp").controller("1012NmsCtrl", [
  "$rootScope",
  "$stateParams",
  "$state",
  "$scope",
  ...
  function ($rootScope, $stateParams, $state, $scope, $timeout, mainService, commonDataSource, Upload, $http, __env) {
    ...
    if($scope.user.user.amgId != null){
      $scope.amgId = $scope.user.user.amgId;
    }else{
      if($scope.user.user.organization != null){
        if($scope.user.user.organization.amgId != null)
          $scope.amgId = $scope.user.user.organization.amgId;
      }
    }
